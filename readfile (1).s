...
calcolo: 
    ...
    cmp %eax , max_i_int             # controllo se ci stanno altre scadenze ,  deve essere (eax<max_i)
    jl  _exit
...
calcolo_2:
    ...
    subl $6, %ebx
    movl $0, %ecx
    movl vettore_int(%ebx), %edx
    movl %edx , vettore_2(%ecx)
    incl %ebx
    incl %ecx
    ...
    subl $6, %ebx
    subl $2, %ecx
    movl vettore_int(%ecx), %edx
    movl %edx , vettore_int(%ebx)
    incl %ebx
    incl %ecx
    movl vettore_int(%ecx), %edx
    movl %edx , vettore_int(%ebx)
