...
in_calcolo:
  ...
  movl $3, %eax   # reinizializzo indice a 3 (il primo valore scadenza salvato in vettore[3])
  movl $10, %ecx
...
calcolo: 
  ...
  # errore 
  addl $4, %eax              # incremento di 4 l'indice per prendere la priorita' sucessiva 
  ...
  movl prec, %ebx
  cmpl (%esi,%eax,4), %ebx    # ebx qui tiene la priorita'   
  jl calcolo_2 
  je scadenza
...
scadenza:   # caso in cui la priorita e' uguale 
  pushl	%eax
  subl $1, %eax
  movl (%esi,%eax,4), %edx
  subl $4, %eax
...
controllo_integrita:
  ...
  # controllo che edi ha inserito un valore tra 1 e 5 nell'ultimo campo se no va fuori
  cmpl	$1, %eax
  jl errore_in
  cmpl	$5, %eax
  jg errore_in  

  # controllo che la scadenza  sia compresa tra 1 e 100
  movl -4(%edi) , %eax
  cmpl	$1, %eax
  jl errore_in
  cmpl	$100, %eax
  ...
  # controllo che la durata sia massimo 10 
  movl -8(%edi) , %eax
  cmpl	$1, %eax
  jl errore_in
  cmpl	$10, %eax
  ...
  # controllo id che sia tra 1 e 127
  movl -12(%edi) , %eax
  cmpl	$1, %eax
  jl errore_in
  cmpl	$127, %eax
